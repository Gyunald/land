...
@st.experimental_memo
def rent(city, date, user_key, rows, monthy):
...
def api2(date,monthy):
    당월전체 = rent(city, date, user_key, rows, monthy)
    return 당월전체
...
try:
    ...
    with st.expander(f'{시군구} 실거래 - {date[4:5+1]}월 🚀 아파트별 {len(아파트별)}건',expanded=True) :
        if len(당월전체) == 0 :
            ...
            아파트별['계약'] = 아파트별['계약'].astype(float).map('{:.2f}'.format)
            st.dataframe(아파트별.style.background_gradient(subset=['금액','면적','계약'], cmap='Reds'),use_container_width=a) 

    with st.expander(f'{시군구} 실거래 - {date[4:5+1]}월 전체 {len(당월전체)}건',expanded=False) :
        if len(당월전체) == 0 :
            st.info(f'{date[4:5+1]}월 신규 등록이 없습니다😎')
        else:
            b = st.radio('선택하세요', ['전용면적','평'])
            if b == '전용면적':                
                st.dataframe(당월전체.style.background_gradient(subset=['금액','면적','계약'], cmap="Reds"),use_container_width=a)
            else:
                당월전체['평'] = (당월전체['면적'].astype(float) / 3.305785).astype(int)+9
                당월전체 = 당월전체.reindex(columns=["아파트", "금액", "층", "평", "건축", "계약", "동", "거래", "파기"])
                st.dataframe(당월전체.style.background_gradient(subset=['금액','평','계약'], cmap="Reds"),use_container_width=a)

    tab1, tab2, tab3 = st.tabs(["매매", "전세", "월세"])
    
    전월당월전세월세 = pd.concat([api2(당월.strftime('%Y%m%d'),0),api2(전월.strftime('%Y%m%d'),0)]).reset_index(drop=True)
    전세_아파트별당월 = 전월당월전세월세[전월당월전세월세['아파트'] == 아파트].reset_index(drop=True)
    당월_전세월세 = 전세_아파트별당월[전세_아파트별당월['계약'].str.contains(date_2)].reset_index(drop=True)
    전세_아파트별 = 전월당월전세월세[전월당월전세월세['아파트'] == 아파트]
    월세_아파트별 = 전월당월전세월세[전월당월전세월세['아파트'] == 아파트]
    ...
    with tab2:
        if len(전세_아파트별) != 0 :
            ...
            st.dataframe(당월_전세월세.style.background_gradient(subset=['보증금','면적','계약'], cmap="Blues"),use_container_width=a)
