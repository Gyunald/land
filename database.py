...
# Initialize Firestore client only once
...
def get_real_estate_data(url, code, user_key, rows, dong, what):
    l = []
    for date in [당월, 전월]:
        url = urls[what]
...
# Main Streamlit app
st.title('Real Estate Data Update')
        ...
        with st.spinner('진행중...'):
            if not db.collection(f"{당월.strftime('%Y.%m.%d')}").document('서울특별시 종로구').get().exists:
                ...
                for dong, code in address.items():
                    thread = Thread(target=get_real_estate_data, args=(urls['매매'], code, user_key, rows, dong, '매매'))
                    threads.append(thread)
                    thread.start()

                    thread = Thread(target=get_real_estate_data, args=(urls['임대'], code, user_key, rows, dong, '임대'))
                    threads.append(thread)
                    thread.start()
                ...
                empty.success('업데이트 완료')
            else:
                st.error('데이터 중복!!!')

    elif login_code and login_code != st.secrets.login_code:
        st.warning('코드 오류')
...
if choice == '삭제':
    ...
    if login_code2 == st.secrets.login_code:
        ...
        for i in list(db.collections())[:-3]:
            ...
            with st.spinner(f"{i.id} 삭제중..."):
                for doc in db.collection(i.id).get():
    ...
    elif login_code2 and login_code2 != st.secrets.login_code:
        st.warning('코드 오류')
