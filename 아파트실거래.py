...
t2 = (datetime.now().replace(hour=6,minute=30,second=0,microsecond=0))

try:
    if standard_str[5:] == (datetime.utcnow()+timedelta(hours=9)).date().strftime('%m.%d') and not t1 <= datetime.now() <= t2 :
