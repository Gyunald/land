...
시군구 = st.selectbox('🍔 시군구 검색', [i for i in address],index=103,label_visibility='collapsed') # 22 강남 103 파주

date = list(db.collections())
day = (datetime.utcnow() + timedelta(hours=9))
try:    
    get_매매 = db.collection(date[-1].id).document(시군구).get().to_dict()['매매']
    temp = 매매(get_매매)
    ...
    if date[-1].id[-2:] == day.strftime('%d'):
        ...
        if len(신규) >= 1:
            ...
            e.write(f"#### :orange[{시군구}] 실거래 {len(신규)}건 ({day.strftime('%m.%d')})")
        ...
        if 전체 :
            ...
            e.write(f"#### :orange[{시군구}] 실거래 {len(매매_당월)}건")
