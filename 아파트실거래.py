...
def 매매(get_매매):
    ...
    if city == '파주시':
        temp['아파트'] = temp['아파트'].apply(lambda j: j[j.index('단지')+2 :] if len(j)/2 > j.index('단지') in j else j)
...
city = st.selectbox('🍔 시군구 검색', [i for i in address],index=103,label_visibility='collapsed') # 22 강남 103 파주
