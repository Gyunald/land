    ...
    if standard_str[5:8] == (datetime.utcnow()+timedelta(hours=9)).date().strftime('%m.%d')[:3] :
        ...
        if standard_str[-2:] == (datetime.utcnow()+timedelta(hours=9)).strftime('%d'):
            ...
            if len(신규) >= 1:
                ...
                st.write(f"#### :orange[{법정동명.split()[-1]}] ({(datetime.utcnow() + timedelta(hours=9)).month}월 전체)")   
            아파트 = st.multiselect('🍞 아파트별',sorted([i for i in 매매_임대["아파트"].drop_duplicates()]),max_selections=3,placeholder= '아파트별 시세 그래프',label_visibility='collapsed')
            # st.warning('🍣 다중선택가능')
        ...
        st.write(f"#### :orange[{법정동명.split()[-1]}] ({(datetime.utcnow() + timedelta(hours=9)).month}월 전체)")   
        아파트 = st.multiselect('🍞 아파트별',sorted([i for i in 매매_임대_계약월별["아파트"].drop_duplicates()]),max_selections=3,placeholder= '아파트별 시세 그래프',label_visibility='collapsed')
        # st.warning('🍣 다중선택가능')
