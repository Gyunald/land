...
try:
    if list(db.collections())[-1].id == (datetime.utcnow()+timedelta(hours=9)).date().strftime('%Y.%m.%d') :        
        매매 = list(db.collection(list(db.collections())[-1].id).document('파주시').get().to_dict()['매매'])
        매매전일 = list(db.collection(list(db.collections())[-2].id).document('파주시').get().to_dict()['매매'])
